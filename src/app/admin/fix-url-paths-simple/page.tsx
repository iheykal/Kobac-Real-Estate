...
export default function FixUrlPathsSimplePage() {
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);

  const fixUrlPaths = async () => {
    setLoading(true);
    setError(null);
    setResults(null);

    try {
      const response = await fetch('/api/admin/fix-url-paths-simple', {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Failed to fix URL paths');
      }
    } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              ...
              <div className="text-sm text-red-700 space-y-2">
                <p><strong>Issue:</strong> Extra `/uploads/` path segment in URLs</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                  <div className="p-2 bg-white rounded border">
                    <div className="font-medium text-red-800">Current URLs (Wrong):</div>
                    <div className="text-xs text-gray-600 break-all">
                      .../kobac-property-uploads/uploads/listings/...
                    </div>
                  </div>
                  <div className="p-2 bg-white rounded border">
                ...
                <p><strong>Result:</strong> URLs return 404 Not Found due to incorrect path structure</p>
              </div>
            </div>

            {/* Solution */}
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <h3 className="font-medium text-blue-800 mb-3">💡 Simple Solution</h3>
              <div className="text-sm text-blue-700 space-y-2">
                <p>This tool will:</p>
                <ul className="ml-4 list-disc space-y-1">
                  <li>Get ALL properties with images (no regex filtering)</li>
                  <li>Check each URL individually for `/uploads/listings/`</li>
                  <li>Replace with `/listings/` if found</li>
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <h3 className="font-medium text-green-800 mb-3">⚡ Fix URL Paths (Simple)</h3>
              <button
                onClick={fixUrlPaths}
                disabled={loading}
                className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? 'Fixing...' : 'Fix URL Paths (Simple)'}
              </button>
            </div>
                ...
                {results.fixed > 0 && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="text-sm text-green-700 space-y-2">
                      <p><strong>URL paths fixed!</strong> Now you should:</p>
                      <ul className="ml-4 list-disc space-y-1">
