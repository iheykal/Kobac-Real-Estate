...
export default function FixBucketMismatchPage() {
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);

  const fixBucketMismatch = async () => {
    setLoading(true);
    setError(null);
    setResults(null);

    try {
      const response = await fetch('/api/admin/fix-bucket-mismatch', {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Failed to fix bucket mismatch');
      }
    } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              ...
              <div className="text-sm text-red-700 space-y-2">
                <p><strong>Issue:</strong> Bucket name mismatch in URLs</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                  <div className="p-2 bg-white rounded border">
                    <div className="font-medium text-red-800">Database URLs (Wrong):</div>
                    <div className="text-xs text-gray-600">kobac252</div>
                  </div>
                  <div className="p-2 bg-white rounded border">
                    <div className="font-medium text-green-800">R2 Config (Correct):</div>
                    <div className="text-xs text-gray-600">kobac-property-uploads</div>
                  </div>
                </div>
                <p><strong>Result:</strong> 6 URLs return 404 Not Found, 2 URLs work correctly</p>
              </div>
            </div>
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div className="text-sm text-blue-700 space-y-2">
                ...
                <ul className="ml-4 list-disc space-y-1">
                  <li>Find all properties with URLs containing "kobac252"</li>
                  <li>Replace "kobac252" with "kobac-property-uploads"</li>
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <h3 className="font-medium text-green-800 mb-3">⚡ Fix Bucket Mismatch</h3>
              <button
                onClick={fixBucketMismatch}
                disabled={loading}
                className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? 'Fixing...' : 'Fix Bucket Mismatch'}
              </button>
            </div>
                ...
                {results.fixed > 0 && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="text-sm text-green-700 space-y-2">
                      <p><strong>Bucket mismatch fixed!</strong> Now you should:</p>
                      <ul className="ml-4 list-disc space-y-1">
                        <li>Go to the properties page and refresh</li>
                        <li>Check if images now load correctly</li>
                        <li>Test uploading a new property image</li>
